...
public class CalendarioController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<CalendarioResponseDTO> update(@Valid @PathVariable Long id, @RequestBody CalendarioUpdateDTO dto) {
    ...
    public ResponseEntity<Void> adicionarFeriado(@Valid @RequestBody itemCalendarioDTO dto){
        service.adicionarFeriado(dto.getCalendarioId(), dto.getItemId());
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/adicionar-programa-educacional")
    public ResponseEntity<Void> adicionarProgramaEducacional(@Valid @RequestBody itemCalendarioDTO dto) {
        service.adicionarProgramaEducacional(dto.getCalendarioId(), dto.getItemId());
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/adicionar-evento-escolar")
    public ResponseEntity<Void> adicionarEventoEscolar(@Valid @RequestBody itemCalendarioDTO dto) {
        service.adicionarEventoEscolar(dto.getCalendarioId(), dto.getItemId());
        return ResponseEntity.noContent().build();
    }

    @DeleteMapping("/remover-feriado")
    public ResponseEntity<Void> removerFeriado(@Valid @RequestBody itemCalendarioDTO dto) {
        service.removerFeriado(dto.getCalendarioId(), dto.getItemId());
        return ResponseEntity.noContent().build();
    }

    @DeleteMapping("/remover-programa-educacional")
    public ResponseEntity<Void> removerProgramaEducacional(@Valid @RequestBody itemCalendarioDTO dto) {
        service.removerProgramaEducacional(dto.getCalendarioId(), dto.getItemId());
        return ResponseEntity.noContent().build();
    }

    @DeleteMapping("/remover-evento-escolar")
    public ResponseEntity<Void> removerEventoEscolar(@Valid @RequestBody itemCalendarioDTO dto) {
    ...
    public ResponseEntity<Page<FeriadoProjection>> findFeriados(@PathVariable Long id, Pageable pageable) {
    ...
    public ResponseEntity<Page<EventoEscolarProjection>> findEventosEscolares(@PathVariable Long id, Pageable pageable) {
        Page<EventoEscolarProjection> eventos = service.findEventosEscolares(id, pageable);
        return ResponseEntity.ok(eventos);
    }

    @GetMapping("/{id}/programas-educacionais")
    public ResponseEntity<Page<ProgramaEducacionalProjection>> findProgramasEducacionais(@PathVariable Long id, Pageable pageable) {
